...
def GetFurtherRefined():
    tail = []

    for each in os.listdir('analysis/'):
        try:
            print('File Name: ' + each)
            fwrite = open('analysis2/' + each , 'w' , encoding = 'utf-8')
            with open('analysis/' + each , 'r' , encoding = 'utf-8') as f:
                reader = csv.reader(f , delimiter = '\t' , quoting = csv.QUOTE_NONE)
                for i , row in enumerate(reader):
                    if str(row[0]).startswith('+92') or \
                            ...
                            str(row[0]).startswith('+124') or \
                            str(row[0]).startswith('Ufone') or \
                            str(row[0]).startswith('MOBILINK') or \
                            str(row[0]).startswith('talkshawk') or \
                            str(row[0]).startswith('Talkshawk') or \
                            str(row[0]).startswith('Telenor') or \
                            str(row[0]).startswith('UPaisa') or \
                            str(row[0]).startswith('Internet') or \
                            str(row[0]).startswith('HEC') or \
                            str(row[0]).startswith('B-SMS') or \
